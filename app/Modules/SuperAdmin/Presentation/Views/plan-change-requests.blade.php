...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-[32px] font-extrabold text-[#111827]">Demandes de Changement de Forfait</h2>
            <p class="text-[15px] text-slate-500 mt-1">Établissements souhaitant passer à un autre forfait.</p>
        </div>
        @if($pendingCount > 0)
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[11px]">
                        ...
                        <th class="py-3.5 px-4">Forfait Actuel</th>
                        <th class="py-3.5 px-4">Forfait Demandé</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($requests as $req)
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="py-4 px-4 font-medium text-slate-500">{{ $req->school->plan_name ?? '—' }}</td>
                            <td class="py-4 px-4 font-bold text-[#031C5B]">{{ $req->requestedPackage->name ?? '—' }}</td>
                            ...
                            <td class="py-4 px-6 text-right">
                                @if($req->status === 'pending')
                                    <div class="flex items-center justify-end gap-2">
                                        <form action="{{ route('superadmin.plan-change-requests.approve', $req->id) }}" method="POST">
                                            @csrf
                                        ...
                                        <form action="{{ route('superadmin.plan-change-requests.reject', $req->id) }}" method="POST">
                                            @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 px-6 text-center text-slate-400 text-[13px]">Aucune demande de changement de forfait pour le moment.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
