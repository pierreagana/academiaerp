...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-5">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <p class="text-[10.5px] text-slate-400 mt-1.5">Présence à l'École</p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <div class="w-9 h-9 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center mb-3"><i class="ph-bold ph-chalkboard-teacher text-[16px]"></i></div>
            <p class="text-[12px] font-bold text-slate-500 mb-1">Présence aux Cours</p>
            <p class="text-[26px] font-extrabold text-[#031C5B] leading-none">{{ $stats['sessions_checked_in'] }} <span class="text-[14px] text-slate-400 font-bold">/ {{ $stats['sessions_scheduled'] }}</span></p>
            <p class="text-[10.5px] text-slate-400 mt-1.5">{{ $stats['sessions_rate'] !== null ? $stats['sessions_rate'] . '% des séances pointées' : 'Aucune séance ce mois-ci' }}</p>
        </div>
    </div>
                ...
                <table class="w-full text-left border-collapse text-[13px]">
                    <thead>
                        <tr class="border-b border-slate-100 text-[10.5px] font-extrabold text-slate-400 uppercase tracking-wider">
                            ...
                            <th class="py-2 text-center">École</th>
                            <th class="py-2 text-right">Cours</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        ...
                        @forelse($applicableRows as $date => $row)
                            @php
                                $coursColor = $row['sessions_scheduled'] == 0
                                    ? 'bg-slate-100 text-slate-400'
                                    : ($row['sessions_checked_in'] >= $row['sessions_scheduled'] ? 'bg-emerald-100 text-emerald-700' : ($row['sessions_checked_in'] > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'));
                            @endphp
                            <tr>
                                ...
                                <td class="py-2.5 text-right">
                                    @if($row['sessions_scheduled'] > 0)
                                        <span class="px-2.5 py-1 rounded-full text-[11px] font-bold {{ $coursColor }}" title="{{ $row['sessions']->map(fn($s) => ($s['subject'] ?? 'Cours') . ' (' . $s['class'] . ', ' . substr($s['start_time'], 0, 5) . ') — ' . ($s['checked_in'] ? 'pointé' : 'non pointé'))->implode(' · ') }}">
                                            {{ $row['sessions_checked_in'] }}/{{ $row['sessions_scheduled'] }} séance(s)
                                        </span>
                                    @else
                        ...
                        @empty
                            <tr><td colspan="6" class="py-6 text-center text-slate-400">Aucun jour prévu ce mois-ci.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
