...
<div class="space-y-6">
            ...
            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" style="display: none;">
                <div @click.away="open = false" class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
                    ...
                    <form action="{{ route('school.finance.fees.config.store') }}" method="POST" class="p-6 space-y-4">
                        @csrf
                        @include('SchoolDashboard::finance._fee_level_fields')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div x-data="{ open: {{ (string) old('_edit_id') === (string) $feeLevel->id ? 'true' : 'false' }} }" class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            ...
            <h3 class="text-xl font-bold text-slate-900">{{ $type === 'tuition' ? $feeLevel->level : \App\Modules\Finance\Domain\Models\FeeLevel::TYPES[$type] }}</h3>
            <div class="mt-3">
            ...
            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" style="display: none;">
                <div @click.away="open = false" class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                        <h3 class="text-[18px] font-extrabold text-[#031C5B]">Modifier la Structure — {{ $type === 'tuition' ? $feeLevel->level : \App\Modules\Finance\Domain\Models\FeeLevel::TYPES[$type] }}</h3>
                        <button type="button" @click="open = false" class="text-slate-400 hover:text-slate-600"><i class="ph-bold ph-x text-lg"></i></button>
                    </div>
                    <form action="{{ route('school.finance.fees.config.update', $feeLevel->id) }}" method="POST" class="p-6 space-y-4">
                        ...
                        @include('SchoolDashboard::finance._fee_level_fields', ['feeLevel' => $feeLevel])
                        <div class="flex justify-end gap-3 pt-2">
                            <button type="button" @click="open = false" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 font-bold text-[14px] rounded-xl hover:bg-slate-50 transition-all">Annuler</button>
