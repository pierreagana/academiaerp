    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('school.academic.homework.tests.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title') }}" placeholder="Ex: Mathématiques Appliquées - Examen Trimestriel" class="w-full bg-[#FAFBFC] border border-slate-200 text-slate-900 text-[14px] rounded-lg px-4 py-2.5 outline-none focus:border-[#031C5B]">
            </div>
            ...
            <div>
                ...
                <input type="number" name="max_score" min="1" max="1000" step="0.5" required value="{{ old('max_score', 20) }}" class="w-32 bg-[#FAFBFC] border border-slate-200 text-slate-900 text-[14px] rounded-lg px-4 py-2.5 outline-none focus:border-[#031C5B]">
